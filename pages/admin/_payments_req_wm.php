<div class="s-bk-lf">
	<div class="acc-title">Список выплат на WebMoney</div>
</div>
...
if(isset($_POST['pay_here']) && is_numeric($_POST['pay_here'])) {
	...
	$db->Query("SELECT * FROM `db_request_payment_wm` WHERE `id` = '$payed' AND `status` = '0' LIMIT 1 ");
        if($db->NumRows() > 0) {
	     $db->Query("UPDATE `db_request_payment_wm` SET `status`='3' WHERE `id`='$payed' ");
        }
}
else if(isset($_POST['canc'])) {
	$payid = intval($_POST['canc']);
	$db->Query("SELECT * FROM `db_request_payment_wm` WHERE `id` = '$payid' AND `status` = '0' LIMIT 1 ");
	if($db->NumRows() > 0){
		...
		$db->Query("UPDATE  db_request_payment_wm SET status = '2' WHERE id = '$payid'");
		$paymentid = $paydata['payment_id'];
		$db->Query("UPDATE  db_payment SET status = '2' WHERE id = '$paymentid'");
	}
}

$db->Query("SELECT * FROM db_request_payment_wm ORDER BY id DESC LIMIT {$lim}, 100");
...
$all_pages = $db->FetchRow();
	...
	echo "<BR /><center>".$nav->Navigation(10, $page, ceil($all_pages / 100), "/?menu=admin384&sel=payments_req_wm&page="), "</center>";
